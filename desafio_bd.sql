...
SELECT TRUNCATE(SUM(s.quantity * p.price), 2) AS total
...
SELECT s.product_id, s.invoice_id, s.quantity, i.customer_id, TRUNCATE(SUM(s.quantity * p.price), 2) AS total
FROM sales s INNER JOIN invoices i ON s.invoice_id = i.id
INNER JOIN products p ON s.product_id = p.id
INNER JOIN customers c ON i.customer_id = c.id
WHERE c.condition = 'True'
GROUP BY s.product_id, s.invoice_id, s.quantity, i.customer_id ORDER BY total DESC limit 5;