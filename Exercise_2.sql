...
select e.title, a.first_name, a.last_name from episodes e inner join actor_episode ae on e.id = ae.episode_id inner join actors a on ae.actor_id = a.id;
...
select mo.title, ac.first_name, ac.last_name from movies mo inner join actor_movie am on am.movie_id = mo.id inner join actors ac on ac.id = am.actor_id where mo.id = 3 or mo.id = 4 group by mo.title, ac.first_name, ac.last_name;